          ...
          <el-table-column align="center" label="操作" width="250px">
            <template slot-scope="scope">
              <div>
                ...
                <span
                  class="cursor color-f8494c"
                  v-if="menuData.review "
                  @click="preview(scope.row.id)"
                >预览</span>
                <span
                  class="cursor color-f8494c"
                  v-if="menuData.scheduing "
                  @click="paiban(scope.row.id)"
                >排班设置</span>
                <span
                  class="cursor color-f8494c"
                  v-if="menuData.appointment_record "
                  @click="book(scope.row.id)"
                >预约记录</span>
              </div>
            </template>
          </el-table-column>
    ...
    >
      <el-form label-width="120px" :model="formLabelAlign">
        ...
        <el-form-item label="* 预约状态">
          <el-select
            v-model="formLabelAlign.scheduling_status"
            clearable
            size="mini"
            placeholder="预约状态"
          >
  ...
  methods: {
      ...
      this.menu.forEach(item => {
        if (item.name == "医生") {
          item.data.forEach(it => {
            if (it.route_web == "/Doctor/DoctorIntroduce") {
    ...
    async edit(id) {
      ...
      if (data.code === 200) {
        ...
        this.fileList = [{ name: "", url: data.data.avatar }];
    ...
    async handleClose() {
      ...
      if (typeof this.formLabelAlign.doctor_details !== "string") {
        this.formLabelAlign.doctor_details = JSON.stringify(
          this.formLabelAlign.doctor_details
        );
      }
      let data = "";
    ...
    empty() {
      ...
      this.timeValue = "";
      (this.params.start_time = ""), (this.params.end_time = ""), this.index();
    },
