  ...
  <div class>
    ...
    <div class="operation">
      ...
      <div class="pull-right">
        <el-button icon="el-icon-circle-plus-outline" v-if="menuData.add" type="primary" size="mini" @click="add">添加排班</el-button>
      </div>
    </div>
    <div class="table">
      <div class="table-list">
        <el-table :data="list" border height="650" :header-cell-style="{background:'#f3f3f3'}">
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <div>
                <span class="cursor color-f8494c" v-if="menuData.edit" @click="edit(scope.row.id)">修改</span>
              </div>
            </template>
          </el-table-column>
        </el-table>
          ...
          >
            ...
            <el-table-column align="center" prop="order_money" label="预约上限人数">
              <template slot-scope="scope">
                <div>
                  <el-input
                    v-model="scope.row.order_max_num"
                    @change="num(scope.row.order_max_num,scope.$index)"
                    placeholder="请输入预约上限"
                  ></el-input>
                </div>
              </template>
            ...
            <el-table-column align="center" prop="order_max_num" label="预约费用">
              <template slot-scope="scope">
                <div>
                  <el-input
                    v-model="scope.row.order_money"
                    @change="val=>money(val,scope.$index)"
                    placeholder="请输入预约费用"
                  ></el-input>
                </div>
              </template>
    ...
    <el-dialog title="修改排班" :visible.sync="editVisible" width="650px" :close-on-click-modal="false">
      <el-form label-width="120px" :model="editData">
        ...
        <el-form-item label="* 预约费用/元">
          <el-input
            v-model="editData.order_money"
            @change="editMoney(editData.order_money)"
            placeholder="请输入预约费用"
          ></el-input>
        </el-form-item>
      </el-form>
