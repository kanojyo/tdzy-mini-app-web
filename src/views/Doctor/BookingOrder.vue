    ...
    <div class>
            ...
            <div class="table-list">
                <el-table :data="list" border height="650" :header-cell-style="{background:'#f3f3f3'}">
                    ...
                    <el-table-column align="center" label="确认就诊时间" width="110px">
                      <template slot-scope="scope">
                        <div>
                          <span class="cursor color-f8494c" v-if="scope.row.status === 2">{{scope.row.handle_at}}</span>
                        </div>
                      </template>
                    </el-table-column>
                    <el-table-column align="center" label="已取消时间" width="110px">
                      <template slot-scope="scope">
                        <div>
                          <span class="cursor color-f8494c" v-if="scope.row.status === 3">{{scope.row.handle_at}}</span>
                        </div>
                      </template>
                    </el-table-column>
        ...
        <el-dialog title="确认取消" :visible.sync="cancelVisible" width="650px">
           <el-form label-width="60px" >
                <el-form-item label="* 备注">
                    <el-input type="textarea" v-model="cancelParams.remarks" placeholder="请输入备注" :maxlength="100"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-dialog :title=title :visible.sync="editVisible" width="650px">
			<table class="ajun-table">
                    ...
                    <tr v-if="title == '确认就诊'">
                        <td>* 备注</td>
                        <td ><el-input type="textarea" v-model="info.remarks" placeholder="请输入备注" :maxlength="100"></el-input></td>
                    </tr>
					<tr v-else>
						<td>备注</td>
						<td>{{info.remarks}}</td>
          ...
          <div class="table">
            <div class="table-list">
                <el-table :data="logList" border height="550" :header-cell-style="{background:'#f3f3f3'}">
                    ...
                    <el-table-column align="center" prop="log_content" label="操作内容" width="300px">
                    </el-table-column>
                </el-table>
            </div>
            <div class="table-peg">
...
import { appointmentList, appointmentConfirm, appointmentCancel, appointmentInfo, appointmentLog} from "@/api/doctor.js";
...
export default {
    data() {
        return {
            ...
            count: 0, //	总数据
            list: [], //	    列表数据
            status: [{ type: 1, value: "未就诊" }, { type: 2, value: "已就诊" }, { type: 3, value: "已取消" }, { type: 4, value: "已失效" }], //  状态
            timeValue:"",
			doctorInfo:{},
			info:{},
    ...
    methods: {
		...
		async confirm(){
			...
			let data = await appointmentConfirm({id:this.info.id,handle_at:this.info.handle_at,remarks:this.info.remarks})
			if(data.code === 200){
