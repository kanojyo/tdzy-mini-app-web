            ...
            <template slot-scope="scope">
              <div>
                <span
                  ...
                  v-if="scope.row.status !== 4 && menuData.process"
                  @click="detailsChange(scope.row.id, 2)"
                >处理</span>&nbsp;
                <span class="cursor color-f8494c" v-if="menuData.details" @click="detailsChange(scope.row.id, 1)">详情</span>&nbsp;
                <span
                  class="cursor color_red"
                  v-if="scope.row.close === 2 && menuData.close"
                  @click="closeChange(scope.row.id)"
                >关闭</span>
              </div>
            </template>
          </el-table-column>
